...
class AddStudentToEventParserTest {
    ...
    void parse_validTutorial_success() throws ParseException {
        assert(new AddStudentToEventParser().parse(VALID_COMMAND_TUTORIAL) instanceof AddStudentToEventCommand);
    }

    @Test
    void parse_validLab_success() throws ParseException {
        assert(new AddStudentToEventParser().parse(VALID_COMMAND_LAB) instanceof AddStudentToEventCommand);
    }

    @Test
    void parse_validConsultation_success() throws ParseException {
        assert(new AddStudentToEventParser().parse(VALID_COMMAND_CONSULTATION) instanceof AddStudentToEventCommand);
    }

    @Test
    void parse_invalidEventField_throwsParseException() {
        assertThrows(ParseException.class, () -> new AddStudentToEventParser().parse(INVALID_EVENT_FIELD_COMMAND));
    }

    @Test
    void parse_tooManyEventFields_throwsParseException() {
        assertThrows(ParseException.class, () -> new AddStudentToEventParser().parse(TOO_MANY_EVENT_FIELDS_COMMAND));
    }

    @Test
    void parse_invalidStudentIndex_throwsParseException() {
        assertThrows(ParseException.class, () -> new AddStudentToEventParser().parse(INVALID_STUDENT_INDEX_COMMAND));
    }
