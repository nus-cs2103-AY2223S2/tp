...
public class AddDoctorCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        Doctor expectedPerson = new PersonBuilder(BOB).withTags(VALID_TAG_FRIEND).buildDoctor();

        // whitespace only preamble
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB
                + NRIC_DESC_BOB + ADDRESS_DESC_BOB + TAG_DESC_FRIEND, new AddDoctorCommand(expectedPerson));
        ...
        assertParseSuccess(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + NRIC_DESC_BOB
                ...
                new AddDoctorCommand(expectedPerson));

        // multiple tags - all accepted
        Doctor expectedPersonMultipleTags = new PersonBuilder(BOB).withTags(VALID_TAG_FRIEND, VALID_TAG_HUSBAND)
                .buildDoctor();
        assertParseSuccess(parser, NAME_DESC_BOB + PHONE_DESC_BOB + EMAIL_DESC_BOB + NRIC_DESC_BOB
                + ADDRESS_DESC_BOB + TAG_DESC_HUSBAND + TAG_DESC_FRIEND,
                new AddDoctorCommand(expectedPersonMultipleTags));
    }

    @Test
    public void parse_optionalFieldsMissing_success() {
        // zero tags
        Doctor expectedPerson = new PersonBuilder(AMY).withTags().buildDoctor();
        assertParseSuccess(parser, NAME_DESC_AMY + PHONE_DESC_AMY + EMAIL_DESC_AMY + NRIC_DESC_AMY
                + ADDRESS_DESC_AMY, new AddDoctorCommand(expectedPerson));
    }
