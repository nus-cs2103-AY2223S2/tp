...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_CCA_POSITION_DESC, CcaPosition.MESSAGE_CONSTRAINTS); // invalid cca Position tag
        ...
        // parsing it together with a valid tag results in error
        assertParseFailure(parser, "1" + CCA_DESC_NES + CCA_DESC_ICS + MODULE_EMPTY,
                Cca.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + CCA_DESC_NES + MODULE_EMPTY + CCA_DESC_ICS,
                Cca.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + MODULE_EMPTY + CCA_DESC_NES+ CCA_DESC_ICS,
                Cca.MESSAGE_CONSTRAINTS);

        // while parsing {@code PREFIX_CCA_POSITION} alone will reset the tags of the {@code Person} being edited,
        // parsing it together with a valid tag results in error
        assertParseFailure(parser, "1" + CCA_POSITION_DESC_PRESIDENT + CCA_POSITION_DESC_DIRECTOR + MODULE_EMPTY,
                CcaPosition.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + CCA_POSITION_DESC_PRESIDENT + MODULE_EMPTY + CCA_POSITION_DESC_DIRECTOR,
                CcaPosition.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, "1" + MODULE_EMPTY + CCA_POSITION_DESC_PRESIDENT + CCA_POSITION_DESC_DIRECTOR,
                CcaPosition.MESSAGE_CONSTRAINTS);
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + PHONE_DESC_BOB + MODULE_DESC_CS2103T + TAG_DESC_HUSBAND + CCA_DESC_NES +
                CCA_POSITION_DESC_PRESIDENT + EMAIL_DESC_AMY + ADDRESS_DESC_AMY + NAME_DESC_AMY + TAG_DESC_FRIEND +
        ...
        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                ...
                .withCcas(VALID_CCA_ICS, VALID_CCA_NES).withCcaPositions(VALID_CCA_POSITION_DIRECTOR, VALID_CCA_POSITION_PRESIDENT)
                .build();
