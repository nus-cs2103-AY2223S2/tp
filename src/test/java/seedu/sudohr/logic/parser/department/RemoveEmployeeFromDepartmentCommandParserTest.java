...
public class RemoveEmployeeFromDepartmentCommandParserTest {
    ...
    public void parse_allFieldsPresent_success() {
        assertParseSuccess(parser, PREAMBLE_WHITESPACE + EID_DESC_AMY + DEPARTMENT_NAME_DESC_ENGINEERING,
                new RemoveEmployeeFromDepartmentCommand(new Id("001"), new DepartmentName("Engineering")));
        assertParseSuccess(parser, EID_DESC_AMY + DEPARTMENT_NAME_DESC_ENGINEERING,
                new RemoveEmployeeFromDepartmentCommand(new Id("001"), new DepartmentName("Engineering")));
    }
    ...
    public void parse_invalidField_failure() {
        ...
        assertParseFailure(parser, VALID_ID_AMY + DEPARTMENT_NAME_DESC_ENGINEERING,
                expectedMessage);
        assertParseFailure(parser, INVALID_EID_DESC + DEPARTMENT_NAME_DESC_ENGINEERING,
                Id.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, EID_DESC_AMY + INVALID_DEPARTMENT_NAME_DESC,
                DepartmentName.MESSAGE_CONSTRAINTS);
        assertParseFailure(parser, INVALID_EID_DESC + INVALID_DEPARTMENT_NAME_DESC, Id.MESSAGE_CONSTRAINTS);
    }
}
