...
public class EditCommandParserTest {
    ...
    public void parse_invalidPreamble_failure() {
        // negative index
        assertParseFailure(parser, " eid/-5" + NAME_DESC_AMY, Id.MESSAGE_CONSTRAINTS);

        // zero index
        assertParseFailure(parser, " eid/0" + NAME_DESC_AMY, Id.MESSAGE_CONSTRAINTS);

        // invalid arguments being parsed as eid
        assertParseFailure(parser, " eid/1 some random string", Id.MESSAGE_CONSTRAINTS);

        // invalid prefix being parsed as eid
        assertParseFailure(parser, " eid/1 i/ string", Id.MESSAGE_CONSTRAINTS);
    }
    ...
    public void parse_someFieldsSpecified_success() {
        ...
        // effectively no change to ID
        userInput = EID_DESC_AMY + ID_DESC_AMY + PHONE_DESC_BOB + EMAIL_DESC_AMY;
        descriptor = new EditEmployeeDescriptorBuilder().withId(VALID_ID_AMY).withPhone(VALID_PHONE_BOB)
                .withEmail(VALID_EMAIL_AMY).build();
        expectedCommand = new EditCommand(new Id(VALID_ID_AMY), descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_oneFieldSpecified_success() {
        String targetIdDesc = VALID_ID_BOB;
        String targetEIdDesc = EID_DESC_BOB;

        // id
        String userInput = targetEIdDesc + ID_DESC_AMY;
        ...
        // name
        userInput = targetEIdDesc + NAME_DESC_AMY;
