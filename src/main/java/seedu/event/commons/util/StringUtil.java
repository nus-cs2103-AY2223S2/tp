package seedu.event.commons.util;
...
public class StringUtil {
    ...
    public static boolean containsKeywordIgnoreCase(String sentence, String word) {
        ...
        checkArgument(preppedWord.split("\\s+").length == 1, "Keyword parameter should be a single keyword");

        String preppedSentence = sentence;
        String[] wordsInPreppedSentence = preppedSentence.split("\\s+");

        return Arrays.stream(wordsInPreppedSentence)
                .anyMatch(curr -> curr.toLowerCase().contains(preppedWord));
    }
