...
public class GCUtil {
    ...
    private static Calendar reconnectService() throws IOException, GeneralSecurityException {
        ...
        service = new Calendar.Builder(HTTP_TRANSPORT, JSON_FACTORY, getCredentials(HTTP_TRANSPORT)).setApplicationName(APPLICATION_NAME).build();
        return service;
    }

    public static List<Event> getEvents(String calendarId, Integer limit, DateTime endsAfter, String order, Boolean expandRecure) throws IOException, GeneralSecurityException {
        Events events = GCUtil.getService().events().list(calendarId).setMaxResults(limit).setTimeMin(endsAfter).setOrderBy(order).setSingleEvents(expandRecure).execute();
