...
public class EditCommandParser implements Parser<EditCommand> {
    ...
    public EditCommand parse(String args) throws ParseException {
        ...
        if (argMultimap.getValue(PREFIX_EMAIL).isPresent()) {
            editPetDescriptor.setEmail(ParserUtil.parseEmail(argMultimap.getValue(PREFIX_EMAIL).get()));
        }
        ...
        if (argMultimap.getValue(PREFIX_TIMESTAMP).isPresent()) {
            editPetDescriptor.setTimeStamp(ParserUtil.parseTimeStamp(argMultimap.getValue(PREFIX_TIMESTAMP).get()));
        }
        if (argMultimap.getValue(PREFIX_DEADLINE).isPresent()) {
            editPetDescriptor.setDeadline(ParserUtil.parseDeadline(argMultimap.getValue(PREFIX_DEADLINE).get()));
        }
