...
import tfifteenfour.clipboard.model.course.exceptions.DuplicateCourseException;
...
public class UniqueCoursesList implements Iterable<Course> {
     ...
     */
    private boolean personsAreUnique(List<Course> students) {
        for (int i = 0; i < students.size() - 1; i++) {
            for (int j = i + 1; j < students.size(); j++) {
                if (students.get(i).isSameCourse(students.get(j))) {
                    return false;
                }
            }
        }
