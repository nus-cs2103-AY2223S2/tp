...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `logic.taa.Logic`.
...
A quick look at the `logic.taa.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
