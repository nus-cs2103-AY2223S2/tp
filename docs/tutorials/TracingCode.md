...
According to the sequence diagram you saw earlier (and repeated above for reference), the `UI` component yields control to the `Logic` component through a method named `execute`. Searching through the code base for an `execute()` method that belongs to the `Logic` component yields a promising candidate in `seedu.company.logic.Logic`.
...
A quick look at the `seedu.company.logic.Logic` (an extract given below) confirms that this indeed might be what we’re looking for.
   ...
   **LogicManager\#execute().**
   ...
   public CommandResult execute(String commandText)
        ...
        try {
            ...
            storage.saveAddressBook(model.getRoleBook());
...
1. The rest of the method seems to exhaustively check for the existence of each possible parameter of the `edit` command and store any possible changes in an `EditRoleDescriptor`. Recall that we can verify the contents of `editRoleDesciptor` through the 'Variables' window.<br>
...
1. As suspected, `command#execute()` does indeed make changes to the `model` object. Specifically,
   * it uses the `setRole()` method (defined in the interface `Model` and implemented in `ModelManager` as per the usual pattern) to update the role data.
   * it uses the `updateFilteredRoleList` method to ask the `Model` to populate the 'filtered list' with _all_ roles.<br>
     FYI, The 'filtered list' is the list of roles resulting from the most recent operation that will be shown to the user immediately after. For the `edit` command, we populate it with all the roles so that the user can see the edited role along with all other roles. If this was a `find` command, we would be setting that list to contain the search results instead.<br>
     To provide some context, given below is the class diagram of the `Model` component. See if you can figure out where the 'filtered list' of roles is being tracked.
...
1. Similar to before, you can step over/into statements in the `LogicManager#execute()` method to examine how the control is transferred to the `Storage` component and what happens inside that component.
   ...
   <div markdown="span" class="alert alert-primary">:bulb: **Intellij Tip:** When trying to step into a statement such as `storage.saveAddressBook(model.getRoleBook())` which contains multiple method calls, Intellij will let you choose (by clicking) which one you want to step into.
   </div>
...
1. It appears that a `JsonAdaptedRole` is created for each `Role` and then added to the `JsonSerializableAddressBook`.
...
2.  What components will you have to modify to perform the following
    ...
    3.  Save the company book in the CSV format instead

    4.  Add a new command

    5.  Add a new field to `Role`

    6.  Add a new entity to the company book
