...
This command allows users of the FitBook application to add optional remarks to people in their FitBook and edit it if required. The command should have the following format:
...
/**
 * Changes the remark of an existing client in the FitBook.
 */
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `FitBookParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
Create a `RemarkCommandParser` class in the `seedu.addressbook.logic.parser` package. The class must extend the `Parser` interface.
...
:information_source: Don’t forget to update `FitBookParser` to use our new `RemarkCommandParser`!
...
Create a new `Remark` in `seedu.addressbook.model.client`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.addressbook.ui.ClientCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
FitBook stores data by serializing `JsonAdaptedClient` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedClient` to work with our new `Client`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete FitBook’s storage file located at `/data/fitbook.json` before running it! Not doing so will cause FitBook to default to an empty FitBook!
...
save it with `FitBookModel#setClient()`.
...
This concludes the tutorial for adding a new `Command` to FitBook.
