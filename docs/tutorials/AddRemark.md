...
This command allows users of the AddressBook application to add optional remarks to people in their company book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/company/logic/command` directory.
...
/**
 * Changes the remark of an existing role in the company book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the role identified "
            + "by the index number used in the last role listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the role in the filtered role list to edit the remark
     * @param remark of the role to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.company.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of role data. We achieve that by working with the `Role` model. Each field in a Role is implemented as a separate class (e.g. a `Name` object represents the role’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a role.

### Add a new `Remark` class

Create a new `Remark` in `seedu.company.model.job`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each role.

Simply add the following to [`seedu.company.ui.RoleCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/RoleListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).
...
## Modify `Role` to support a `Remark` field

Since `RoleCard` displays data from a `Role`, we need to update `Role` to get our `Remark` displayed!

### Modify `Role`

We change the constructor of `Role` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Role`

Unfortunately, a change to `Role` will cause other commands to break, you will have to modify these commands to use the updated `Role`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Role` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedRole` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedRole` to work with our new `Role`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty company book!
...
Now that we have finalized the `Role` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public RoleCard(Role role, int displayedIndex) {
    //...
    remark.setText(role.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Role`. Since all fields in a `Role` are immutable, we create a new instance of a `Role` with the values that we want and
save it with `Model#setRole()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Role: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Role: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Role editedRole = new Role(
                roleToEdit.getName(), roleToEdit.getPhone(), roleToEdit.getEmail(),
                roleToEdit.getAddress(), remark, roleToEdit.getTags());

        model.setRole(roleToEdit, editedRole);
        model.updateFilteredRoleList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedRole));
    }
     ...
     * {@code roleToEdit}.
     */
    private String generateSuccessMessage(Role roleToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, roleToEdit);
    }
```
