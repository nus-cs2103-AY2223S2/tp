...
Let's walk you through the implementation of a new command — `comment`.
...
`comment INDEX r/REMARK` (e.g., `comment 2 r/Likes baseball`)
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `AddressBookParser#parseCommand()` to recognize the `comment` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
![Output displayed](../images/add-comment/RemarkHello.png)
...
![The relationship between RemarkCommand and Command](../images/add-comment/CommandInterface.png)
...
public class RemarkCommand extends Command {
     ...
     */
    public RemarkCommand(Index index, String comment) {
...
Now let’s move on to writing a parser that will extract the index and comment from the input provided by the user.

Create a `RemarkCommandParser` class in the `seedu.address.logic.parser` package. The class must extend the `Parser` interface.

![The relationship between Parser and RemarkCommandParser](../images/add-comment/ParserInterface.png)
...
This appears to be what we need to get a String of the comment. But what about the Index? Let's take a quick peek at existing `Command` that uses an index to see how it is done.
...
public RemarkCommand parse(String args) throws ParseException {
    ...
    String comment = argMultimap.getValue(PREFIX_REMARK).orElse("");

    return new RemarkCommand(index, comment);
}
```
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of person data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the person’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a comment given to a person.
...
## Add a placeholder element for comment to the UI
...
![$comment shows up in each entry](../images/add-comment/$Remark.png)
...
public PersonCard(Person person, int displayedIndex) {
    //...
    comment.setText(person.getRemark().value);
}
```

![The comment label is bound properly!](../images/add-comment/RemarkBound.png)
...
``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added comment to Person: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed comment from Person: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
    ...
    private String generateSuccessMessage(Person personToEdit) {
        String message = !comment.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, personToEdit);
    }
```

![Congratulations!](../images/add-comment/RemarkComplete.png)
...
Let’s create a test for `RemarkCommand#execute()` to test that adding a comment works. On `IntelliJ IDEA` you can bring up the context menu and choose to `Go To` \> `Test` or use the appropriate keyboard shortcut.

![Using the context menu to jump to tests](../images/add-comment/ContextMenu.png)

Then, create a test for the `execute` method.

![Creating a test for `execute`.](../images/add-comment/CreateTest.png)
