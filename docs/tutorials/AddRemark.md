...
/**
 * Changes the remark of an existing fish in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the fish identified "
            + "by the index number used in the last fish listing. "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the fish in the filtered fish list to edit the remark
     * @param remark of the fish to be updated to
     */
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of fish data. We achieve that by working with the `Fish` model. Each field in a Fish is implemented as a separate class (e.g. a `Name` object represents the fish’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a fish.

### Add a new `Remark` class

Create a new `Remark` in `seedu.address.model.fish`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each fish.

Simply add the following to [`seedu.address.ui.FishCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
Then insert the following into [`main/resources/view/FishListCard.fxml`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-d44c4f51c24f6253c277a2bb9bc440b8064d9c15ad7cb7ceda280bca032efce9).
...
## Modify `Fish` to support a `Remark` field

Since `FishCard` displays data from a `Fish`, we need to update `Fish` to get our `Remark` displayed!

### Modify `Fish`

We change the constructor of `Fish` to take a `Remark`. We will also need to define new fields and accessors accordingly to store our new addition.

### Update other usages of `Fish`

Unfortunately, a change to `Fish` will cause other commands to break, you will have to modify these commands to use the updated `Fish`!

<div markdown="span" class="alert alert-primary">

:bulb: Use the `Find Usages` feature in IntelliJ IDEA on the `Fish` class to find these commands.
...
AddressBook stores data by serializing `JsonAdaptedFish` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedFish` to work with our new `Fish`!
...
Now that we have finalized the `Fish` class and its dependencies, we can now bind the `Remark` field to the UI.
...
public FishCard(Fish fish, int displayedIndex) {
    //...
    remark.setText(fish.getRemark().value);
}
```
...
In this last step, we modify `RemarkCommand#execute()` to change the `Remark` of a `Fish`. Since all fields in a `Fish` are immutable, we create a new instance of a `Fish` with the values that we want and
save it with `Model#setFish()`.

**`RemarkCommand.java`:**

``` java
//...
    public static final String MESSAGE_ADD_REMARK_SUCCESS = "Added remark to Fish: %1$s";
    public static final String MESSAGE_DELETE_REMARK_SUCCESS = "Removed remark from Fish: %1$s";
//...
    @Override
    public CommandResult execute(Model model) throws CommandException {
        ...
        Fish editedFish = new Fish(
                fishToEdit.getName(), fishToEdit.getPhone(), fishToEdit.getEmail(),
                fishToEdit.getAddress(), remark, fishToEdit.getTags());

        model.setFish(fishToEdit, editedFish);
        model.updateFilteredFishList(PREDICATE_SHOW_ALL_Fishes);

        return new CommandResult(generateSuccessMessage(editedFish));
    }
     ...
     * {@code fishToEdit}.
     */
    private String generateSuccessMessage(Fish fishToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, fishToEdit);
    }
```
