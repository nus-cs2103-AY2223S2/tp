...
/**
 * Changes the remark of an existing card in the address book.
 */
...
``` java
    public static final String MESSAGE_USAGE = COMMAND_WORD
            + ": Edits the remark of the card identified "
...
public class RemarkCommand extends Command {
    ...
    /**
     * @param index of the card in the filtered card list to edit the remark
     * @param remark of the card to be updated to
     */
...
Create a `RemarkCommandParser` class in the `seedu.powercards.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of card data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the card’s name). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a card.

### Add a new `Remark` class

Create a new `Remark` in `seedu.powercards.model.card`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Without getting too deep into `fxml`, let’s go on a 5 minute adventure to get some placeholder text to show up for each card.

Simply add the following to [`seedu.powercards.ui.CardElement`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
``` java
public PersonCard(Person card, int displayedIndex) {
    //...
    remark.setText(card.getRemark().value);
}
```
...
//...
    ...
    public CommandResult execute(Model model) throws CommandException {
        ...
        Person cardToEdit = lastShownList.get(index.getZeroBased());
        Person editedCard = new Person(
                cardToEdit.getName(), cardToEdit.getPhone(), cardToEdit.getEmail(),
                cardToEdit.getAddress(), remark, cardToEdit.getTags());

        model.setPerson(cardToEdit, editedCard);
        model.updateFilteredPersonList(PREDICATE_SHOW_ALL_PERSONS);

        return new CommandResult(generateSuccessMessage(editedCard));
    }
     ...
     * {@code cardToEdit}.
     */
    private String generateSuccessMessage(Person cardToEdit) {
        String message = !remark.value.isEmpty() ? MESSAGE_ADD_REMARK_SUCCESS : MESSAGE_DELETE_REMARK_SUCCESS;
        return String.format(message, cardToEdit);
    }
```
