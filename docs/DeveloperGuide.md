...
The UI consists of a `MainWindow` that is made up of parts e.g.`CommandBox`, `ResultDisplay`, `DeckListPanel`,`CardListPanel` `StatusBarFooter` etc. All `Panel` named components act as containers for `element` classes that act as individual objects created by `Model`. There are some components that are mutually exclusive, being `DeckListPanel` with `ReviewStatsPanel` and`UnflippedReviewElement` with `FlippedReviewElement`.
