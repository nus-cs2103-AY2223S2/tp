...
2. Deleting a person while persons with a specified keyword, i.e., `KEYWORD` of the user's choice, are being shown
    ...
    1. Prerequisites: List all persons using the `list` command, then find all persons with `[KEYWORD]` using the `find KEYWORD [KEYWORD]...` command. Multiple persons in the list.
