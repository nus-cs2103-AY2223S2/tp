...
Moreover, AB3 code base only consists of features supporting only `Person` class, meanwhile for Duke Driver, we had to work with and update the code base to support different entity types - for example, `Delivery Job` and `Reminder`. With the extensions that we planned to do, we also had to update the `Parser`, `Model` and `Logic` to support a much larger set of commands, as we were working with numerous commands from different windows (Timetable, Reminder, Statistics,...).
